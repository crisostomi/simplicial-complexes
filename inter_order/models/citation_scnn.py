class MySCNN(nn.Module):
    ...
    """
        ...
        assert component_to_use == "both" or keep_separated

        self.colors = colors
        self.aggregation = aggregation
        self.component_to_use = component_to_use
    ...
    """
        ...
        if self.keep_separated:
            ...
            outs["l0"] = {
                f"d{dim}": {comp: xs[dim] for comp in comps[f"d{dim}"]} for dim in dims
            }
            ...
            outs["l0"] = {f"d{dim}": xs[dim] for dim in dims}
